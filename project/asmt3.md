...
| 特性             | 对应测试用例                                                                                                                | 涉及改动                                                 |
| -------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **浮点类型 (f32)** | `float_basic`, `float_arith`, `float_cmp`, `float_cast`, `float_func`                                                 | `conversions.rs`, `type_infer.rs`, `function_gen.rs` |
...
共 20 个测试用例。每个对应 `tests/<name>/<name>.tea` 源程序和 `tests/<name>/<name>.ir` 期望输出。我们需要让 `teac --emit ir <file>` 对所选的特性涉及的所有测试文件都能成功输出 IR。
...
[LLVM](https://llvm.org/) 是一套模块化的编译器基础设施，其核心设计理念是将编译过程分为**前端**（源语言 → IR）和**后端**（IR → 目标机器码）两个独立阶段，中间通过一种统一的**中间表示（Intermediate Representation, IR）**衔接。
...
**浮点常量**在 LLVM IR 中以 IEEE 754 双精度十六进制格式表示。例如 `3.14` 表示为 `0x40091EB860000000`，`1.0` 表示为 `0x3FF0000000000000`。即使操作类型是 `float`（单精度），常量仍以双精度十六进制写入，LLVM 在编译时自动截断。
...
LLVM IR 采用 **静态单赋值（Static Single Assignment, SSA）**形式：每个虚拟寄存器（`%r0`, `%r1`, ...）在整个函数中**只被定义一次**。这个约束使得数据流分析和优化变得简单，但也带来一个问题——当同一个变量在不同控制流路径上被赋予不同的值时，如何在合流点确定使用哪个值？
...
`FloatConst` 内部以 `f64` 存储值，打印时输出 IEEE 754 双精度十六进制表示（`0x{:016X}`），这是 LLVM IR 的标准浮点常量格式（参见 2.4 节）。
...
这些类型和语句结构已经在 IR 层定义好了。本实验需要在 IR **生成**阶段正确地产生这些指令。

### 3.2 涉及的改动

`conversions.rs`：AST 类型到 IR 类型的转换。需要确保 `ast::BuiltIn::Float` 被正确映射到 `Dtype::F32`，以及浮点数组类型能被正确处理。

`type_infer.rs`：前向类型推断器。需要识别 `f32` 类型的变量声明和定义，支持 `as` 表达式的类型推断（`x as f32` 的结果类型是 `f32`，`y as i32` 的结果类型是 `i32`）。

`function_gen.rs`：函数体 IR 生成。这是改动量最大的文件，需要处理：

1. **浮点字面量**：将 AST 中的浮点字面量转为 `Operand::FloatConst`
2. **浮点运算**：当操作数是 `f32` 时，生成 `FBiOp`（`fadd`/`fsub`/`fmul`/`fdiv`）而非 `BiOp`（`add`/`sub`/`mul`/`sdiv`）
3. **浮点比较**：当操作数是 `f32` 时，生成 `FCmp`（`fcmp oeq`/`one`/...）而非 `Cmp`（`icmp eq`/`ne`/...）
4. **类型转换**：`as` 表达式生成 `SIToFP` 或 `FPToSI` 指令
