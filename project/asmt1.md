...
开放任务：自行实现新语法特性，或对 teac 做出有意义的改进（被合并进主分支）
...
float_basic, float_arith, float_cmp, float_cast, float_func



Grammar, AST Types, Parser
...
float_cast, float_arith, float_func



Grammar, AST Expr, Parser
...
array_2d_basic, array_2d_init, array_2d_matmul, array_3d, attention
...
for_basic, for_continue, for_mixed, for_nested, for_range



Grammar, AST Stmt, Parser
...
struct_method_basic, struct_method_calls, struct_method_loop, struct_method_namespace, struct_method_nested



Grammar, AST Program/Expr/Stmt, Parser
...
解析成功：teac --emit ast <file> 退出码为 0
...
AST 非空：stdout 包含非空的 AST 输出



关键标识符存在：AST 中包含源程序中的关键函数名、结构体名、变量名
...
不需要单独的 lexer：PEG 是"无扫描器"（scannerless）的，即关键字、运算符、标识符的识别和语法解析在同一层完成。不需要像 flex + bison 那样分成词法分析和语法分析两个阶段。



备选项的顺序至关重要：在 PEG 中，调换 α 和 β 的顺序可能改变解析结果。编写 PEG 规则时，必须仔细考虑各备选项的排列顺序。
...
"for" identifier "in" rangeBound ".." rangeBound "{" codeBlockStmt* "}"
...
identifier exprSuffixNonMethod* "." identifier "(" rightValList? ")"
...
新增 floatLiteral 和 -floatLiteral 选项
...
Parser 结构体、Generator trait 实现、ParseContext 定义、解析入口
...
Error 枚举、TeaLangParser（Pest 派生）、类型别名、辅助函数
...
声明与定义的解析（use 语句、变量声明/定义、函数声明/定义、结构体定义）
...
语句的解析（赋值、调用、if、while、return、break、continue）
...
Pest 解析：TeaLangParser::parse(Rule::program, input) 将源代码转换为 Pest 的解析树（一棵由 Pair 节点构成的树）。



AST 构建：ParseContext 上的方法遍历 Pest 解析树，将其转换为 teac 自定义的 AST 节点。
...
parse_fn_def、parse_struct_def、parse_var_decl、parse_type_spec
...
parse_arith_expr、parse_bool_expr、parse_fn_call、parse_right_val
...
parse_if_stmt、parse_while_stmt、parse_assignment_stmt
...
grammar_error、parse_num、get_pos
...
BuiltIn、TypeSpecifier、TypeSpecifierInner
...
ArithBiOp、ComOp、BoolBiOp、BoolUOp
...
ArithExpr、ExprUnit、BoolExpr、FnCall、LeftVal、RightVal 等
...
IfStmt、WhileStmt、AssignmentStmt、CodeBlockStmt 等
...
VarDecl、VarDef、FnDecl、FnDef、ParamDecl、StructDef
...
Program、ProgramElement、UseStmt
...
DisplayAsTree trait 的实现（树状缩进表示）
...
打印自身：调用 tree_indent 获取缩进前缀，写入节点名称和关键信息。



构造子缩进：将当前的 is_last 推入 indent_levels，形成新的缩进上下文。



递归打印子节点：最后一个子节点传 is_last = true，其他传 false。
...
在 display.rs 中实现 Display：提供单行文本表示。



在 tree.rs 中实现 DisplayAsTree：提供树状缩进表示。
